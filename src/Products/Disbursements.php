...
class Disbursements extends MomoApp
{
	...
	public function requestToken($apiUserId){
		...
		$request=new Request("GET",MomoLinks::D_TOKEN_URI,$this->headers);
		return $this->send($request);
	}
	public function transferStatus($resourceId){
		$this->setAuth();
		$request=new Request("GET",MomoLinks::D_TRANSFER_URI.'/'.$resourceId,$this->headers);
		return $this->send($request);
	}
	public function acountHolder($accountHolderIdType,$accountHolderId){
		$request=new Request("GET",MomoLinks::D_ACCOUNT_HOLDER_URI.$accountHolderIdType.'/'.$accountHolderId.'/active',$this->headers);
		return $this->send($request);
	}
	public function transfer(RequestToPay $requestBody,$ref,$callbackUri=false){
		...
		$request=new Request("POST",MomoLinks::D_TRANSFER_URI,$this->headers,$requestBody->generateRequestBody());
		return $this->send($request);
	}
	public function requestBalance(){
		// $this->setAuth();
		$this->removeHeader(Constants::H_AUTH);
		$request=new Request("GET",MomoLinks::D_BALANCE_URI,$this->headers);
		return $this->send($request);
